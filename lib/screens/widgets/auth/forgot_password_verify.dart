...
class VerifiCationFields {
  ...
  factory VerifiCationFields.fromJson(Map<String, dynamic> json) {
    return VerifiCationFields(
        phone: json['phone'], code: json['code'], isCodeVerified: json['isCodeVerified']);
  }
}
            ...
            children: [
              Expanded(
                ...
                append: SizedBox(
                    width: 97,
                    child: CustomButton(
                      labelText: phone.isNotEmpty && errorMessages.isEmpty && isVerification
                          ? '재발송'
                          : '인증받기',
                      backgroundColor: phone.isNotEmpty && errorMessages.isEmpty
                      ...
                      onPressed: () async {
                        AuthApi().sendCode(phone).then((value) {
                          ...
                          // Message
                          String message = phone.isNotEmpty && errorMessages.isEmpty
                              ? '인증번호를 재발송 했어요'
                              : '인증번호가 발송되었습니다';
                          // Toast
                          CustomToast.successToast(context, message,
                ...
                InputTextField(
                  ...
                  append: SizedBox(
                      ...
                      child: CustomButton(
                        labelText: '확인',
                        backgroundColor:
                            code.isNotEmpty ? const Color(0XFF555555) : const Color(0XFFBBBBBB),
                        textColor: Colors.white,
                        onPressed: () async {
                          AuthApi().verifyCode(phone, code).then((value) {
                            ...
                            if (value['code'] == 200) {
                              setState(() {
                                isCodeVerified = true;
                                CustomToast.successToast(context, '인증코드가 확인되었습니다',
                                    bottom: MediaQuery.of(context).size.height * 0.12);
                              });
                            } else {
                              CustomToast.errorToast(context, value['message'],
                                  bottom: MediaQuery.of(context).size.height * 0.12);
                            }
                          }).catchError((error) {
