...
class LodgingStep03 extends StatefulWidget {
  double deviceWidth;
  LodgingStep03({super.key, required this.deviceWidth});
...
class _LodgingStep03State extends State<LodgingStep03> {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        children: [
          ContentDescription(presentPage: 3, totalPage: 6, description: '선호하는 숙소 주변 환경을\n순서대로 선택해 주세요',),
          SizedBox(height: 24.0.h),
          Consumer<TasteProvider>(
              builder: (context, data, child) {

                List<bool> selectorState = data.lodgingEnvSelector;
                ...
                return Column(
                  children: [
                    Row(
                        ...
                        children: [
                          lodgingEnvBox(widget.deviceWidth, 0, null, '번화가 근처', selectorState[0]),
                          lodgingEnvBox(widget.deviceWidth, 1, null, '관광지 근처', selectorState[1]),
                          lodgingEnvBox(widget.deviceWidth, 2, null, '독채', selectorState[2]),
                        ]
                    ),
                    SizedBox(height: 24.0.h),
                    Row(
                        ...
                        mainAxisAlignment: MainAxisAlignment.spaceAround,
                        children: [
                          lodgingEnvBox(widget.deviceWidth, 3, null, '자연친화적', selectorState[3]),
                          lodgingEnvBox(widget.deviceWidth, 4, null, '교통/역세권', selectorState[4]),
                          lodgingEnvBox(widget.deviceWidth, 5, null, '도심', selectorState[5]),
                        ]
                    ),
                  ],
                );
  ...
  Widget lodgingEnvBox(double width, int index, [String? image, String? title, bool? state]) {
    ...
    return GestureDetector(
      onTap: () {
        context.read<TasteProvider>().lodgingEnvSelectorChange(index, !profileState);
      },
      child: Column(
        children: [
          Container(
