              ...
              children: [
                Expanded(
                  child: GestureDetector(
                    onTap: () {
                      context.read<FeedProvider>().commentModelRequest(
                          feedModel!.id!, context.read<FeedProvider>().sortState);
                      ...
                      showModalBottomSheet(
                          ...
                          builder: (context) {
                            ...
                            return CommentView(
                                postId: feedModel!.id, bottomPadding: widget.bottomPadding);
                          });
                    },
                    ...
                    child: Container(
                      ...
                      child: Row(children: [
                        commentState == true
                            ...
                            : Image.asset('assets/feed/my_comment_empty.png',
                                width: 22, height: 22),
            ...
            children: [
              ...
              Material(
                ...
                child: InkWell(
                  ...
                  onTap: () async {
                    ...
                    if (data.recommentMode == false) {
                      ...
                      if (data.updateMode == false) {
                        ...
                        responseModel = await CommentRequest()
                            .commentWriteRequest(widget.postId, controller.text);
                        ...
                        if (isRegedit == true) {
                          responseModel = await CommentRequest()
                              .commentUpdateRequest(selectModel.id!, controller.text);
                          if (responseModel == CommentResponseModel()) {
                            /// update fail
                          } else {
                            /// update success
                      ...
                      if (data.updateMode == false) {
                        ...
                        responseModel = await CommentRequest()
                            .recommentWriteRequest(data.selectCommentModel.id!, controller.text);
                        ...
                        /// 답글상태 -> 수정일때
                        if (isRegedit == true) {
                          if (context.read<FeedProvider>().isRecommentOption == true) {
                            /// 답글상태 -> 답글 수정
                            responseModel = await CommentRequest()
                                .commentUpdateRequest(childModel.childCommentId!, controller.text);
                            if (responseModel == CommentResponseModel()) {
                              /// update fail
                            } else {
                              /// update success
                              controller.clear();
                              isRegedit == false;
                              context.read<FeedProvider>().isRecommentOptionInit();

                              /// 답글 옵션 선택 초기화
                              context.read<FeedProvider>().updateSuccess(widget.postId, sortState);
                            }
                          } else if (context.read<FeedProvider>().isRecommentOption == false) {
                            /// 답글상태 -> 댓글 수정
                            responseModel = await CommentRequest()
                                .commentUpdateRequest(selectModel.id!, controller.text);
                            if (responseModel == CommentResponseModel()) {
                  ...
                  child: Consumer<FeedProvider>(builder: (context, data, child) {
                    return Center(
                        child: Image.asset('assets/feed/comment_regedit.png',
                            width: 24,
                            height: 24,
                            color: data.inputButton == true
