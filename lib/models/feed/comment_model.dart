...
class CommentRequest {
  Future<List<CommentResponseModel>> commentRequest(int postId, String sort) async {
    final response = await http.get(
      Uri.parse('${ApiUrl.releaseUrl}/post/${postId.toString()}/comments?ordering=$sort'),
      headers: {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      List<CommentResponseModel> commentModels =
          body.isEmpty ? [] : body.map((e) => CommentResponseModel.fromJson(e)).toList();
  ...
  Future<CommentResponseModel> commentWriteRequest(int postId, String comment) async {
    ...
    final response = await http.post(
      Uri.parse('${ApiUrl.releaseUrl}/post/${postId.toString()}/comment'),
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      CommentResponseModel model =
          CommentResponseModel.fromJson(jsonDecode(utf8.decode(response.bodyBytes))['data']);
      return model;
    } else {
      print('댓글 입력 실패');
  ...
  Future<CommentResponseModel> commentUpdateRequest(int commentId, String comment) async {
    ...
    final response = await http.patch(
      Uri.parse('${ApiUrl.releaseUrl}/comment/${commentId.toString()}'),
      body: jsonEncode(commentRequestBody),
      headers: {
        'Authorization': 'Bearer ${PresentUserInfo.loginToken}',
        'Content-Type': 'application/json; charset=UTF-8'
  ...
  Future<CommentResponseModel> recommentWriteRequest(int parentCommentId, String comment) async {
    ...
    final response = await http.post(
      Uri.parse('${ApiUrl.releaseUrl}/comment/${parentCommentId.toString()}/comment'),
      body: jsonEncode(commentRequestBody),
      headers: {
        'Authorization': 'Bearer ${PresentUserInfo.loginToken}',
        'Content-Type': 'application/json; charset=UTF-8'
  ...
  Future<CommentResponseModel> commentLikeRequest(int commentId) async {
    ...
    final response = await http.post(
      Uri.parse('${ApiUrl.releaseUrl}/comment/${commentId.toString()}/like'),
      headers: {
        'Authorization': 'Bearer ${PresentUserInfo.loginToken}',
        'Content-Type': 'application/json; charset=UTF-8'
      },
  ...
  Future<CommentResponseModel> commentUnlikeRequest(int commentId) async {
    ...
    final response = await http.post(
      Uri.parse('${ApiUrl.releaseUrl}/comment/${commentId.toString()}/unlike'),
      headers: {
        'Authorization': 'Bearer ${PresentUserInfo.loginToken}',
        'Content-Type': 'application/json; charset=UTF-8'
      },
...
class CommentResponseModel {
  ...
  CommentResponseModel.fromJson(dynamic json) {
    ...
    json['child_comments'] == [] || json['child_comments'] == null
        ? childCommentList = []
